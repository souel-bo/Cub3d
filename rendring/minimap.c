...
void	draw_minimap(t_mlx *all)
{
	...
	for (int y = 0; y < MINIMAP_SIZE; y++)
		for (int x = 0; x < MINIMAP_SIZE; x++)
			if (minimap_x + x < WIN_WIDTH && minimap_y + y < WIN_HEIGHT)
				all->map->pixels[(minimap_y + y) * WIN_WIDTH + (minimap_x
						+ x)] = 0x000000;
	...
	for (int row = start_row; row <= end_row; row++)
	{
		...
		for (int col = start_col; col <= end_col; col++)
		{
			...
			float screen_x_f = minimap_x + (relative_x + MINIMAP_RADIUS)
				* TILE_MINI_MAP;
			float screen_y_f = minimap_y + (relative_y + MINIMAP_RADIUS)
				* TILE_MINI_MAP;
	...
	if (player_size < 3)
		player_size = 3;
	draw_square(all, player_screen_x - player_size / 2, player_screen_y
	...
	for (int i = 0; i < line_length; i++)
	{
		...
		if (px >= minimap_x && px < minimap_x + MINIMAP_SIZE && py >= minimap_y
			&& py < minimap_y + MINIMAP_SIZE && px < WIN_WIDTH
			&& py < WIN_HEIGHT)
	...
	for (int i = 0; i < MINIMAP_SIZE; i++)
	{
		if (minimap_y < WIN_HEIGHT && minimap_x + i < WIN_WIDTH)
			all->map->pixels[minimap_y * WIN_WIDTH + (minimap_x
					+ i)] = 0x808080;
		if (minimap_y + MINIMAP_SIZE - 1 < WIN_HEIGHT && minimap_x
			+ i < WIN_WIDTH)
			all->map->pixels[(minimap_y + MINIMAP_SIZE - 1) * WIN_WIDTH
				+ (minimap_x + i)] = 0x808080;
		if (minimap_y + i < WIN_HEIGHT && minimap_x < WIN_WIDTH)
			all->map->pixels[(minimap_y + i) * WIN_WIDTH
				+ minimap_x] = 0x808080;
		if (minimap_y + i < WIN_HEIGHT && minimap_x + MINIMAP_SIZE
			- 1 < WIN_WIDTH)
			all->map->pixels[(minimap_y + i) * WIN_WIDTH + (minimap_x
					+ MINIMAP_SIZE - 1)] = 0x808080;
	}
}